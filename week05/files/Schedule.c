...
bool ScheduleAdd(Schedule s, Time t) {
    ...
    if(ceiling == NULL){
        ...
        if(TimeCmp(floor, t) > 0 || TimeCmp(floor, t) == 0){
            return false;
        }
        else{
            TreeInsert(s->times, t);
        ...
        if(TimeCmp(ceiling, t) < 0 || TimeCmp(ceiling, t) == 0){
            return false;
        }
        else{
            TreeInsert(s->times, t);
    ...
    ceiling = TimeSubtractMinutes(ceiling, 10);
    floor = TimeAddMinutes(floor, 10);
    if(TimeCmp(ceiling, t) < 0 || TimeCmp(floor, t) > 0 || TimeCmp(ceiling, t) == 0 || TimeCmp(floor, t) == 0){
        return false;
    }
    else{
        TreeInsert(s->times, t);
